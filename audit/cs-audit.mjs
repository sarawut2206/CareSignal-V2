...
function layer7() {
  ...
  /* ---- X-104: ด่านกันระบบสั่งตัวเอง ต้องอยู่หน้าการตัดสินใจเสียงเสมอ ----
     ...
     ด่านที่ต้องมี: หน้าต่างเวลาที่ระบบกำลังพูด (SPK.until) + VAD ในเครื่อง
     ตรวจการต่อสายแบบตรงตัวขึ้นบรรทัด — regex หลวม ๆ เคยปล่อยให้โค้ดที่ถูก
     คอมเมนต์ทิ้ง (//vadStart();) ผ่านการตรวจ ซึ่งคือการปิดด่านโดยตัวตรวจไม่รู้ */
  {
    const bad = APP_PAIR.filter(([f, t]) => {
      const okFns = /function\s+vcBlockedBy\s*\(/.test(t) && /function\s+vadVoicedMs\s*\(/.test(t)
        && /function\s+vadStart\s*\(/.test(t) && /\n\s*vadStart\(\);/.test(t);
    ...
    req(7, "X-104", "ด่านกันระบบสั่งตัวเอง (หน้าต่าง TTS + VAD) ต่อไว้หน้า vcDispatch",
  ...
  /* ---- X-107: เส้นทางเสียงต้องไม่ส่งเสียงหรือข้อความออกนอกเครื่อง ----
     ผู้ใช้เคยเสนอให้ต่อ VAD/ถอดเสียงแบบคลาวด์ (ai-coustics · Krisp · Soniox)
     ซึ่งต้องส่งเสียงผู้สูงอายุออกไปประมวลผลที่เซิร์ฟเวอร์ภายนอก ขัดกับที่
     ประกาศไว้กับกรรมการและกับผู้ใช้ กฎนี้กันไม่ให้หลุดเข้ามาโดยไม่ตั้งใจ */
  {
    ...
    req(7, "X-107", "การรู้จำเสียงและ VAD ทำในเครื่อง ไม่ส่งเสียงออกนอกเครื่อง",
        bad.length ? "FAIL" : "PASS",
        bad.length ? ("พบการอ้างบริการเสียงภายนอกใน " + bad.join(" · "))
                   : "ใช้ Web Speech API ของเบราว์เซอร์และ VAD ในเครื่องเท่านั้น");
    if (bad.length) finding("CRITICAL", "X-107", "เสียงผู้ใช้ถูกส่งออกไปประมวลผลภายนอก",
      "ขัดกับคำมั่นเรื่องความเป็นส่วนตัวที่ประกาศไว้ทั้งในแอปและในเอกสารประกวด",
      bad.join(" · "), "ถอดการเรียกบริการเสียงภายนอกออก ใช้การประมวลผลในเครื่องเท่านั้น");
  }
