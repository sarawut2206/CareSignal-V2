...
function layer7() {
     ...
     และแถวเกินกำหนดจะไม่โดดออกมา — สามชิ้นร่วมนี้ต้องอยู่ครบทุกหน้า */
  {
    ...
    /* หน้า "งานของฉัน" ใช้ชื่อกระดานตามบทบาท (bd?bd.nm:"งานของฉัน") จึงรับทั้งสองแบบ */
    ["งานวันนี้", '(?:bd\\?bd\\.nm:)?"งานของฉัน"', "ติดตามการส่งต่อ", "ทบทวนยา", "พอร์ตความเสี่ยง", "บันทึกตรวจสอบ", "จัดการผู้ใช้"].forEach((t) => {
      const pat = t.startsWith("(") ? t : '"' + t + '"';
      if (!new RegExp('head\\(' + pat + ',[^;]*,\\s*(rows|q|n)(\\.length)?\\)').test(st)) bad.push("หน้า " + t.replace(/[(?:)\\"]|bd\?bd\.nm:/g, "") + " ไม่ได้ส่งจำนวนรายการเข้าหัวหน้า");
    });
  ...
  /* ---- X-130: ชุดสัมภาษณ์ผู้ใช้ และใบสรุปที่ส่งประกวด ----
     ใบสรุปหนึ่งหน้าต้องคำนวณจาก validation/results.json เท่านั้น ไม่มีตัวเลขพิมพ์มือ
     และต้องกันการส่งแบบเปล่าด้วยธง filled ไม่งั้นวันส่งจริงอาจเผลอแนบไฟล์เปล่า
     ที่ตัวเลขเป็นศูนย์ทั้งใบ · ไฟล์ข้อมูลดิบห้ามมีชื่อจริงของผู้ให้ข้อมูล
     ชุดสัมภาษณ์ต้องยังบังคับหลักถามปัญหาก่อนบอกไอเดีย ซึ่งเป็นเหตุผลเดียวที่ผลจะเชื่อถือได้ */
  {
    const bad = [];
    const kit = read("tools/make-validation-kit.mjs") || "";
    const sum = read("tools/make-validation-summary.mjs") || "";
    const raw = read("validation/results.json") || "";
    if (!kit) bad.push("ไม่มีชุดสัมภาษณ์ (tools/make-validation-kit.mjs)");
    if (!sum) bad.push("ไม่มีตัวสร้างใบสรุป (tools/make-validation-summary.mjs)");
    if (!raw) bad.push("ไม่มีไฟล์ข้อมูลดิบ validation/results.json");
    if (!/ห้ามพูดถึง CareSignal/.test(kit) || !/อย่าอธิบายไอเดียก่อนถามปัญหา/.test(kit))
      bad.push("ชุดสัมภาษณ์ไม่ได้บังคับให้ถามปัญหาก่อนบอกไอเดีย");
    if (!/ยินยอมให้ถ่ายภาพ/.test(kit) || !/ไม่ใช่ชื่อจริง/.test(kit))
      bad.push("ชุดสัมภาษณ์ไม่มีใบยินยอมถ่ายภาพ หรือไม่ได้ใช้รหัสแทนชื่อจริง");
    if (!/d\.filled === true && n > 0/.test(sum))
      bad.push("ใบสรุปไม่ได้กันการส่งแบบเปล่าด้วยธง filled");
    if (!/ยังไม่ใช่ข้อมูลจริง ห้ามส่งใบนี้/.test(sum))
      bad.push("ใบสรุปไม่มีแถบเตือนเมื่อยังไม่ได้กรอกข้อมูลจริง");
    if (/const FEATURES = \[/.test(sum))
      bad.push("ใบสรุปประกาศรายการฟีเจอร์เอง ต้องอ่านจากชุดสัมภาษณ์ที่เดียว");
    if (!/import \{ FEATURES \} from "\.\/make-validation-kit\.mjs"/.test(sum))
      bad.push("ใบสรุปไม่ได้อ่านรายการฟีเจอร์จากชุดสัมภาษณ์");
    if (/"(name|fullname|phone|address)"/.test(raw) || /ชื่อจริง"\s*:/.test(raw))
      bad.push("ไฟล์ข้อมูลดิบมีช่องชื่อจริงหรือเบอร์โทร");
    try {
      const j = JSON.parse(raw || "{}");
      if (Array.isArray(j.people) && j.people.length > 0 && j.filled !== true)
        bad.push("มีข้อมูลผู้ให้ข้อมูลแล้วแต่ยังไม่ตั้ง filled เป็น true ใบสรุปจะขึ้นแถบห้ามส่ง");
      (j.people || []).forEach((x) => {
        if (x && x.code && !/^R\d{2,}$/.test(String(x.code)))
          bad.push("รหัสผู้ให้ข้อมูล " + x.code + " ไม่ใช่รูปแบบ R01");
      });
    } catch (e) { bad.push("validation/results.json อ่านเป็น JSON ไม่ได้"); }
    req(7, "X-130", "ชุดสัมภาษณ์บังคับถามปัญหาก่อนบอกไอเดีย และใบสรุปคำนวณจากข้อมูลดิบ กันการส่งแบบเปล่า",
        bad.length ? "FAIL" : "PASS",
        bad.length ? bad.join(" · ")
                   : "ชุดสัมภาษณ์มีใบยินยอมและใช้รหัสแทนชื่อ · ใบสรุปอ่านฟีเจอร์จากที่เดียว นับจาก results.json และขึ้นแถบแดงเมื่อยังไม่ได้กรอกจริง");
  ...
  /* ---- X-132: หน้าเว็บนำเสนอต้องหาเรื่องที่ต้องการเจอ ไม่ใช่เลื่อนหาเอง ----
     ส่วนแนะนำยาวราวห้าเท่าของหนึ่งจอ กรรมการและผู้สนใจที่เปิดครั้งแรก
     ไม่มีทางรู้ว่ามีอะไรอยู่ข้างล่าง จึงต้องมีสารบัญที่กระโดดถึงได้ในคลิกเดียว
     รายการยาวต้องจัดกลุ่มพับได้ (ห้ามตัดข้อความทิ้งเพื่อให้สั้น — ข้อจำกัดของระบบ
     ต้องยังอ่านได้ครบ) และลิงก์ที่แชร์ต่อต้องพาไปถึงหัวข้อนั้นจริง ไม่เด้งกลับหน้าแรก */
  {
    const bad = [];
    const idx = read("index.html") || "";
    const toc = (idx.match(/<nav class="toc"[\s\S]*?<\/nav>/) || [""])[0];
    const links = [...toc.matchAll(/<a href="#([a-z]+)"\s+data-sec="([a-z]+)">/g)].map((m) => [m[1], m[2]]);
    if (links.length < 5) bad.push("สารบัญมีน้อยกว่า 5 หัวข้อ");
    for (const [href, sec] of links) {
      if (href !== sec) bad.push("ลิงก์สารบัญ #" + href + " ชี้คนละที่กับ data-sec=" + sec);
      if (!new RegExp('id="' + href + '"[^>]*data-toc="1"').test(idx))
        bad.push("สารบัญชี้ไปยัง #" + href + " ที่ไม่มีอยู่จริงหรือไม่ได้เว้นที่ใต้แถบติดหน้าจอ");
    }
    if ((idx.match(/data-toc="1"/g) || []).length !== links.length)
      bad.push("มีหัวข้อที่ทำเครื่องหมายไว้แต่ไม่ได้อยู่ในสารบัญ");
    if (!/\.toc\{position:sticky;top:var\(--toc-top,var\(--nav-h\)\)/.test(idx) || !/setProperty\("--toc-top"/.test(idx))
      bad.push("สารบัญไม่ได้ติดใต้แถบเมนูตามความสูงจริง");
    if (!/aria-current", a\.dataset\.sec === cur/.test(idx)) bad.push("สารบัญไม่ไฮไลต์หัวข้อที่กำลังอ่าน");
    /* แถบเมนูบนต้องติดจริงตลอดหน้า — body ที่ถูกล็อกความสูงเท่าจอทำให้ sticky ตายหลังจอแรก */
    if (!/body\{min-height:100%\}/.test(idx) || /html,body\{height:100%\}/.test(idx))
      bad.push("body ถูกล็อกความสูงเท่าจอ แถบเมนูจะติดได้แค่จอแรก");
    /* รายการยาว: จัดกลุ่มได้ แต่ต้องครบทุกข้อ */
    const doList = (idx.match(/<h4>ระบบทำอะไร<\/h4>[\s\S]*?<\/div>/) || [""])[0];
    const grp = [...doList.matchAll(/<details class="acc"( open)?>\s*<summary>[^<]+<span class="cnt">(\d+) ข้อ<\/span>/g)];
    const per = [...doList.matchAll(/<details class="acc"[\s\S]*?<\/details>/g)].map((m) => (m[0].match(/<li>/g) || []).length);
    if (grp.length < 3) bad.push("รายการยาวยังไม่ได้จัดกลุ่มให้พับได้");
    if (grp.filter((g) => g[1]).length !== 1) bad.push("ต้องเปิดกลุ่มแรกไว้กลุ่มเดียว ไม่ใช่ปิดหมดหรือเปิดหมด");
    if (grp.map((g) => +g[2]).join() !== per.join()) bad.push("จำนวนข้อที่เขียนบนหัวกลุ่มไม่ตรงกับของจริง");
    if (per.reduce((a, b) => a + b, 0) < 25) bad.push("ข้อความในรายการหายไป เหลือ " + per.reduce((a, b) => a + b, 0) + " จาก 25 ข้อ");
    if (/<div class="card no">[\s\S]*?<details/.test(idx)) bad.push("รายการ ระบบไม่ทำอะไร ถูกพับซ่อน — ข้อจำกัดต้องเห็นโดยไม่ต้องกด");
    /* แถบลำดับ 8 ขั้นตอน ต้องตรงกับการ์ดจริงทั้งเลขและสถานะบังคับ */
    const rail = [...idx.matchAll(/<a href="#st(\d)"( class="op")?><b>(\d)<\/b><span>([^<]+)<\/span><\/a>/g)];
    const cards = [...idx.matchAll(/<div class="gs" id="st(\d)"><div class="gi n">(\d)<\/div><div><b>([^<]+)<\/b> <span class="gt">([^<]+)<\/span>/g)];
    if (rail.length !== 8 || cards.length !== 8) bad.push("แถบลำดับหรือการ์ดขั้นตอนไม่ครบ 8 ขั้น");
    else for (let i = 0; i < 8; i++) {
      if (rail[i][1] !== String(i + 1) || cards[i][1] !== String(i + 1)) bad.push("ลำดับขั้นตอนที่ " + (i + 1) + " ไม่เรียงกัน");
      if (!!rail[i][2] !== /ไม่บังคับ/.test(cards[i][4]))
        bad.push("ขั้นที่ " + (i + 1) + " แถบลำดับบอกบังคับ/ข้ามได้ ไม่ตรงกับการ์ด");
      if (cards[i][3].indexOf(rail[i][4].replace(/ \(.*/, "")) < 0)
        bad.push("ชื่อย่อขั้นที่ " + (i + 1) + " ไม่ตรงกับชื่อในการ์ด");
    }
    /* ลิงก์ลึกต้องพาไปถึงจริง และแฮชมั่วต้องไม่ทำให้หน้าว่าง */
    if (!/var el = document\.getElementById\(h\), pan = el && el\.closest\("\.panel"\)/.test(idx)
        || !/el\.scrollIntoView\(\{ block: "start" \}\)/.test(idx))
      bad.push("ลิงก์ลึกไปยังหัวข้อย่อยยังเด้งกลับหน้าแรก");
    if (!/if \(!pan\) \{ show\("intro", false\); return; \}/.test(idx)) bad.push("แฮชที่ไม่มีอยู่จริงอาจทำให้หน้าว่าง");
    req(7, "X-132", "หน้าเว็บนำเสนอมีสารบัญ รายการยาวพับได้ครบทุกข้อ แถบลำดับตรงกับการ์ด และลิงก์ลึกพาไปถึงจริง",
        bad.length ? "FAIL" : "PASS",
        bad.length ? bad.join(" · ")
                   : "สารบัญ 5 หัวข้อไฮไลต์ตามที่อ่าน · รายการ 25 ข้อจัด 5 กลุ่ม ข้อจำกัดยังเปิดให้เห็น · แถบลำดับ 8 ขั้นตรงกับการ์ด · #bizmodel และ #st4 เปิดถึงหน้าที่ถูกต้อง");
    if (bad.length) finding("MED", "X-132", "คนอ่านหาเรื่องที่ต้องการในหน้าเว็บไม่เจอ หรือข้อความหายไปจากการย่อหน้า",
      "กรรมการมีเวลาอ่านจำกัด ถ้าเลื่อนหาเองห้าจอกว่าจะเจอโมเดลรายได้ ก็อาจไม่เจอเลย และถ้าย่อหน้าด้วยการตัดข้อความทิ้ง ข้อจำกัดที่ประกาศไว้จะหายไปด้วย",
      bad.join(" · "), "คงสารบัญและกลุ่มพับได้ไว้ ถ้าเพิ่มหัวข้อใหม่ให้ใส่ id + data-toc และเพิ่มลิงก์ในสารบัญด้วย");
  }

  /* ---- X-133: เดโมเส้นทางหนึ่งเคส ต้องอ่านจากชุดข้อมูลสาธิตชุดเดียวกับคอนโซล ----
     เดโมบนเวทีเล่าเรื่องคนเดียวตั้งแต่ครอบครัวแจ้งเหตุจนถึงตัวเลขของบริษัทประกัน
     ถ้าหน้าเล่าเรื่องมีตัวเลขของตัวเอง สักวันจะไม่ตรงกับคอนโซลที่กรรมการกดเปิดดู
     จึงต้องอ่านจาก CS_DEMO.state() เท่านั้น เคสเดินเรื่องต้องมีอยู่จริงในข้อมูล
     ครบทั้งสี่วิชาชีพ ติดป้ายสาธิตเหมือนหน้าอื่น และห้ามพาเข้าแอปสมาชิกในโหมดสาธิต */
  {
    const bad = [];
    const J = read("CareSignal-Journey.html") || "", d = read("cs-demo.js") || "", ix = read("index.html") || "", sw = read("sw.js") || "";
    if (!J) bad.push("ไม่มี CareSignal-Journey.html");
    if (J.indexOf('src="./cs-demo.js"') < 0 || J.indexOf('src="./cs-demo.js"') < J.indexOf('src="./cs-backend.js')) bad.push("หน้าเดโมไม่ได้โหลดโหมดสาธิตหลัง backend");
    if (!/if \(!window\.CS_DEMO\)[\s\S]*p\.set\("demo", "1"\)/.test(J)) bad.push("เปิดหน้าเดโมโดยไม่มี ?demo แล้วไม่บังคับเข้าโหมดสาธิต");
    if (/supabase|CS_CONFIG|esm\.sh|fetch\(/.test(J)) bad.push("หน้าเดโมอ้างถึงฐานข้อมูลจริง");
    if (!/CS_DEMO\.state\(\)/.test(J) || /ftsst_seconds: 1[0-9]\./.test(J)) bad.push("หน้าเดโมมีตัวเลขของตัวเอง ไม่ได้อ่านจากชุดข้อมูลสาธิต");
    if (!/CSBackend\.insurerOutcomes\(\)/.test(J)) bad.push("ตัวเลขบริษัทประกันไม่ได้มาจากฟังก์ชันเดียวกับแดชบอร์ด");
    if (/CareSignal-App\.html\?[^"']*demo/.test(J)) bad.push("หน้าเดโมพาเข้าแอปสมาชิกในโหมดสาธิต");
    if (!/flagship: true/.test(d) || !/pseudonym: "DEMO-41"/.test(d)) bad.push("ชุดข้อมูลสาธิตไม่มีเคสเดินเรื่อง DEMO-41");
    for (const dest of ["pharmacist", "doctor", "physio", "nurse"])
      if (!new RegExp('\\["' + dest + '", \\d+, \\d+, [^\\]]*"outcome_recorded"|\\["' + dest + '", \\d+, \\d+,').test(d)) bad.push("เคสเดินเรื่องไม่มีใบส่งต่อถึง " + dest);
    if (!/status: "outcome_recorded",\s*\n?\s*reply_due/.test(d) && !/status: "outcome_recorded"/.test(d)) bad.push("ใบส่งต่อของเคสเดินเรื่องยังไม่บันทึกผลหลังรับบริการ");
    if (!/refer_doctor/.test(d) || !/ไม่หยุดทันที/.test(d)) bad.push("เภสัชกรในเดโมสั่งหยุดยาเอง แทนที่จะส่งต่อแพทย์");
    if (!/ไม่ใช่การวินิจฉัย/.test(J) || !/ไม่มีการคิดเบี้ยหรือพิจารณาสิทธิ์/.test(J)) bad.push("หน้าเดโมไม่บอกขอบเขต (ไม่วินิจฉัย ไม่คิดเบี้ย)");
    if ((ix.match(/CareSignal-Journey\.html\?demo=1/g) || []).length < 2) bad.push("หน้าหลักไม่บอกทางเข้าหน้าเดโมเส้นทางหนึ่งเคส");
    if (sw.indexOf("./CareSignal-Journey.html") < 0) bad.push("service worker ไม่ precache หน้าเดโม");
    req(7, "X-133", "เดโมเส้นทางหนึ่งเคสอ่านจากชุดข้อมูลสาธิตชุดเดียวกับคอนโซล ครบสี่วิชาชีพ ติดป้ายสาธิต และไม่แตะแอปสมาชิก",
        bad.length ? "FAIL" : "PASS",
        bad.length ? bad.join(" · ") : "DEMO-41 เดินครบวงจรในข้อมูลสาธิต · หน้าเดโมอ่านจาก CS_DEMO.state() และ insurerOutcomes() · ปุ่มเปิดหน้าจอจริงชี้ไปใบส่งต่อนั้นในบทบาทนั้น");
    if (bad.length) finding("MED", "X-133", "เดโมเส้นทางหนึ่งเคสอาจเล่าไม่ตรงกับหน้าจอจริง หรือปนข้อมูลสังเคราะห์เข้าแอปสมาชิก",
      "กรรมการที่กดเปิดหน้าจอจริงจากหน้าเล่าเรื่องแล้วเห็นตัวเลขคนละชุด จะไม่เชื่อทั้งสองหน้า",
      bad.join(" · "), "ให้หน้าเดโมอ่านจาก CS_DEMO.state() เท่านั้น และคงเคส DEMO-41 ไว้ใน cs-demo.js ครบทุกขั้น");
  }

  /* ---- X-134: แดชบอร์ดบริษัทประกันต้องตอบว่า "ต้องทำอะไรต่อ" ไม่ใช่แค่รายงานตัวเลข ----
     ของเดิมเรียงตามชนิดข้อมูล (ประชากร → ความเสี่ยง → สัญญาณ → การดูแล → ผลลัพธ์)
     และเมื่อฐานข้อมูลว่างก็ขึ้น 0 ราย · 0% ทุกบรรทัดโดยไม่บอกว่าทำไม ซึ่งอ่านได้
     ทั้งว่า "ระบบพัง" และ "ไม่มีความเสี่ยงเลย" — คนละความหมายกันโดยสิ้นเชิง
     รุ่นนี้เรียงเป็น 4 ชั้นตามลำดับการตัดสินใจ และต้องคงสามอย่างนี้ไว้เสมอ
       (1) สถานะว่างที่อธิบายได้ แทนศูนย์เปล่า
       (2) ป้ายบอกที่มาของข้อมูล (สาธิต/จริง) พร้อมเวลาอัปเดต
       (3) บริษัทประกันไม่มีทางกดไปดูรายบุคคล — ปุ่มคิวงานขึ้นเฉพาะทีมดูแล
     และห้ามมีกลไกฝั่งประกัน (เบี้ย ส่วนลด ระยะรอคอย การพิจารณารับประกัน) กลับมา */
  {
    const bad = [];
    const pf = read("CareSignal-Portfolio-Dashboard.html") || "";
    const be = read("cs-backend.js") || "", dm = read("cs-demo.js") || "", sql = read("supabase/21_insurer_trend.sql") || "";
    /* โครง 4 ชั้น เรียงตามลำดับการตัดสินใจ */
    const order = ["kpis", "riskmix", "todo", "s-trend", "s-cycle"].map((k) => pf.indexOf('id="' + k + '"'));
    if (order.some((v) => v < 0)) bad.push("โครงสี่ชั้นไม่ครบ (ตัวเลข · ความเสี่ยง · งานค้าง · แนวโน้ม · วงจร)");
    else if (!order.every((v, i) => i === 0 || v > order[i - 1])) bad.push("สี่ชั้นเรียงผิดลำดับการตัดสินใจ");
    if (/>ก · |>ข · |>ค · |>ง · |>จ · /.test(pf)) bad.push("กลับไปใช้หัวข้อแบบเอกสารราชการ ก · ข · ค");
    /* (1) สถานะว่าง */
    if (!/function emptyState\(\)/.test(pf) || !/if\(!o\.n_members\)\{emptyState\(\);return\}/.test(pf))
      bad.push("ฐานข้อมูลว่างแล้วยังขึ้นศูนย์ทุกบรรทัด ไม่มีสถานะว่างที่อธิบายได้");
    /* (2) ที่มาข้อมูลและเวลา */
    if (!/ข้อมูลสาธิต<\/span>/.test(pf) || !/ข้อมูลจริงจากฐานข้อมูล/.test(pf) || !/อัปเดตล่าสุด/.test(pf))
      bad.push("ไม่ได้บอกว่าเป็นข้อมูลสาธิตหรือของจริง หรือไม่บอกเวลาอัปเดต");
    /* (3) สิทธิ์ */
    if (!/ROLE&&ROLE!=="insurer"\?'<a class="tbtn pri"/.test(pf)) bad.push("ปุ่มคิวงานไม่ได้จำกัดเฉพาะบัญชีที่ไม่ใช่บริษัทประกัน");
    if (/pseudonym|display_name|insurerPortfolio\(/.test(pf)) bad.push("แดชบอร์ดดึงข้อมูลรายบุคคลเข้ามา");
    if (!/ปิดตัวเลขเมื่อกลุ่มเล็กกว่า 10 คน/.test(pf)) bad.push("ไม่ได้ประกาศการกดเซลล์เล็กกว่า 10 คน");
    /* กลไกฝั่งประกันที่ระบบไม่ได้ทำ */
    for (const w of ["premium", "discount", "loss ?ratio", "underwrit", "actuarial"])
      if (new RegExp(w, "i").test(pf)) bad.push("มีกลไกฝั่งประกันกลับมา: " + w);
    for (const w of ["เบี้ย", "ส่วนลด", "ระยะรอคอย"]) {
      const hits = [...pf.matchAll(new RegExp("[^<>]{0,40}" + w + "[^<>]{0,20}", "g"))].map((m) => m[0]);
      if (hits.some((h) => !/ไม่/.test(h))) bad.push("คำว่า " + w + " ถูกใช้นอกประโยคที่บอกว่าไม่ทำ");
    }
    if (!/จะไม่แสดงข้อความว่าลดการเคลมได้/.test(pf)) bad.push("ไม่ได้ประกาศว่ายังไม่อ้างผลลดการเคลม");
    /* แนวโน้มรายเดือน — ต่อครบทั้งสามฝั่ง */
    if (!/create or replace view public\.insurer_monthly/.test(sql) || !/where public\.cs_role\(\) in \('insurer', 'care_manager', 'admin'\)/.test(sql))
      bad.push("view insurer_monthly หายไปหรือไม่ได้บังคับสิทธิ์ด้วย cs_role");
    if (!/from\("insurer_monthly"\)/.test(be) || !/insurerMonthly: insurerMonthly,/.test(be)) bad.push("backend ไม่ได้ต่อ insurer_monthly");
    if (!/insurerMonthly: function \(\)/.test(dm)) bad.push("โหมดสาธิตไม่มี insurerMonthly เดโมจะไม่มีกราฟ");
    if (!/typeof CSBackend\.insurerMonthly==="function"/.test(pf)) bad.push("หน้าจอไม่ได้กันกรณี backend ยังไม่มีฟังก์ชันนี้");
    /* กราฟ: เขียนเอง ไม่โหลดไลบรารีนอก และแยกสีชุดข้อมูลออกจากสีสถานะ */
    if (!/function lineChart\(/.test(pf) || !/function stackChart\(/.test(pf)) bad.push("กราฟแนวโน้มหายไป");
    if (/cdn\.|unpkg|chart\.js/i.test(pf)) bad.push("โหลดไลบรารีกราฟจากภายนอก ขัดกับ CSP ของหน้า");
    if (!/--s1:#1D4E9A; --s2:#B45309/.test(pf)) bad.push("สีชุดข้อมูลของกราฟไม่ได้แยกจากสีสถานะความเสี่ยง");
    /* ส่วน "ฐานข้อมูลการดูแลก่อนเกิดเคลม" — อธิบายว่าบริษัทประกันได้อะไร
       จุดที่พังง่ายคือเคลมข้อมูลที่ยังไม่มี ทุกแถวจึงต้องติดป้ายว่ามีแล้วหรือยังต้องเชื่อมเคลม
       และตารางผลิตภัณฑ์ต้องบอกว่ายังไม่มีบริษัทเข้าร่วม ไม่ใช่เขียนเหมือนขายได้แล้ว */
    if (pf.indexOf('id="s-value"') < 0) bad.push("ไม่มีส่วนอธิบายว่าบริษัทประกันได้ข้อมูลอะไร");
    else {
      const vsec = pf.slice(pf.indexOf('id="s-value"'), pf.indexOf('class="pending"'));
      const live = (vsec.match(/tag live/g) || []).length, wait = (vsec.match(/tag wait/g) || []).length;
      if (live < 5 || wait < 2) bad.push("ตารางคุณค่าไม่ได้ติดป้ายว่าข้อมูลใดมีแล้วและข้อมูลใดต้องเชื่อมเคลมก่อน");
      if (!/ค่าสินไหม[\s\S]{0,400}tag wait/.test(vsec)) bad.push("อ้างว่ามีข้อมูลค่าสินไหมแล้ว ทั้งที่ยังไม่ได้เชื่อม");
      if (!/ยังไม่มีบริษัทประกันเข้าร่วมโครงการ/.test(vsec)) bad.push("ตารางผลิตภัณฑ์ไม่ได้บอกว่ายังไม่มีบริษัทเข้าร่วม");
      if (!/ไม่ใช้เพื่อปฏิเสธการจ่ายสินไหม/.test(vsec)) bad.push("ไม่ได้ประกาศว่าข้อมูลนี้ห้ามใช้ปฏิเสธสินไหม");
    }
    req(7, "X-134", "แดชบอร์ดบริษัทประกันเรียงตามลำดับการตัดสินใจ มีสถานะว่างที่อธิบายได้ บอกที่มาข้อมูล และไม่เปิดทางดูรายบุคคล",
        bad.length ? "FAIL" : "PASS",
        bad.length ? bad.join(" · ")
                   : "สี่ชั้นครบและเรียงถูก · สถานะว่างแทนศูนย์เปล่า · ป้ายสาธิต/จริง + เวลาอัปเดต · ปุ่มคิวงานเฉพาะทีมดูแล · กราฟรายเดือนต่อครบทั้ง view, backend และโหมดสาธิต");
    if (bad.length) finding("HIGH", "X-134", "แดชบอร์ดกลับไปเป็นหน้ารายงานที่อ่านไม่ออกว่าต้องทำอะไรต่อ หรือเปิดทางให้เห็นรายบุคคล",
      "ผู้บริหารบริษัทประกันเปิดหน้ามาแล้วตอบไม่ได้ว่ามีอะไรต้องจัดการวันนี้ หรือแย่กว่านั้นคือเห็นข้อมูลรายบุคคลที่ไม่ควรเห็นตามหลัก data minimization",
  ...
  /* ---- X-141: บทบาทคือ คัดกรอง · เฝ้าระวัง · ส่งต่อ — ไม่อ้างว่าระบบ "ป้องกัน" เอง ----
     การป้องกันการล้มเป็นงานของผู้เชี่ยวชาญ (กายภาพ แพทย์ เภสัชกร ปรับบ้าน) ที่ระบบส่งต่อไปถึง
     คำว่า "บริการป้องกัน / แผนป้องกัน / โปรแกรมป้องกัน / ป้องกันก่อนเคลม" ในข้อความที่ผู้ใช้เห็น
     ทำให้ถูกตีความว่าระบบรับประกันผล (ความรับผิดเมื่อผู้ใช้ล้ม) และคล้ายสิทธิประโยชน์เพิ่มในกรมธรรม์
     คงไว้ได้: ชื่อหน่วยงาน (กองป้องกันการบาดเจ็บ · เวชศาสตร์ป้องกัน) คำอธิบายยา และชื่อตาราง/ฟังก์ชันในฐานข้อมูล */
  {
    const bad = [];
    ...
    ["index.html", "CareSignal-App.html", "CareSignal-Vision.html", "CareSignal-Staff.html", "CareSignal-Portfolio-Dashboard.html",
     "cs-teleconsult.js", "cs-demo.js", "cs-roi.js", "cs-referral-forms.js"].forEach((f) => {
      const t = (read(f) || "").replace(/\/\*[\s\S]*?\*\//g, "");   /* คอมเมนต์ที่อ้างถ้อยคำเดิมเพื่ออธิบายไม่นับ */
      const m = t.match(BANNED); if (m) bad.push(f + ": \"" + m[0] + "\"");
    });
    req(7, "X-141", "ข้อความที่ผู้ใช้เห็นบอกบทบาทเป็น คัดกรอง · เฝ้าระวัง · ส่งต่อ ไม่อ้างว่าระบบป้องกันการล้มเอง",
        bad.length ? "FAIL" : "PASS",
        bad.length ? bad.join(" · ") : "ไม่มี บริการ/แผน/โปรแกรมป้องกัน หรือ ป้องกันก่อนเคลม ในหน้าเว็บ แอป คอนโซล แดชบอร์ด และโมดูลใช้ร่วม");
    if (bad.length) finding("MED", "X-141", "ข้อความอ้างว่าระบบป้องกันการล้มเอง",
      "ระบบคัดกรองและส่งต่อ การป้องกันเป็นงานของผู้เชี่ยวชาญ ถ้อยคำว่าป้องกันทำให้ถูกตีความว่ารับประกันผล และคล้ายสิทธิประโยชน์ในกรมธรรม์",
      bad.join(" · "), "ใช้ คัดกรอง · เฝ้าระวัง · ส่งต่อ · ลดความเสี่ยง · บริการที่ผู้เชี่ยวชาญแนะนำ");
  }
     ...
     ผลยืนยันต้องมีผู้รับผิดชอบที่มีใบอนุญาตและแก้ไขย้อนหลังไม่ได้ และระบบไม่บันทึกภาพหรือเสียงจากวิดีโอคอล */
  {
    ...
    if (!/insurer:\s*\["port","prev","me"\]/.test(st) || /(pharmacist|physio|doctor|nurse|care_manager):\s*\[[^\]]*"prev"/.test(st)) bad.push("เมนูคำขออนุมัติบริการแสดงผิดบทบาท");
    if (!/\.side \[hidden\],\.tabs \[hidden\][^{]*\{display:none!important\}/.test(st)) bad.push("เมนูที่ไม่มีสิทธิ์ยังแสดงอยู่ (display ทับ hidden)");
    if (!/ใช้ข้อมูลนี้อนุมัติบริการที่ผู้เชี่ยวชาญแนะนำเท่านั้น/.test(st) || !/ห้ามนำไปปรับเบี้ย/.test(st)) bad.push("หน้าบริษัทประกันไม่ประกาศขอบเขตการใช้ข้อมูล");
  ...
  /* ---- X-136: แยกกระดานตามวิชาชีพ ไม่ให้ทุกคนเห็นคิวเดียวกัน ----
     ...
     (CDC STEADI Coordinated Care Plan) คอนโซลจึงต้อง
       (1) ให้เลือกบทบาทก่อนเข้าสู่ระบบ และบอกว่าการเลือกไม่ได้ให้สิทธิ์
       (2) ตั้งชื่อกระดานและบอกขอบเขตของบทบาทนั้นทุกหน้า
       (3) ไม่มีเคสของบทบาท = บอกตรง ๆ ไม่เอาของวิชาชีพอื่นมาแสดงแทน
       (4) การกรองต้องอยู่ที่ฐานข้อมูล ไม่ใช่การซ่อนบนหน้าจอ */
  {
    const bad = [];
    const st = read("CareSignal-Staff.html") || "", sql = read("supabase/12_roles.sql") || "";
    for (const k of ["doctor", "pharmacist", "physio", "nurse"])
      if (!new RegExp("\\b" + k + ":\\{ic:").test(st)) bad.push("ไม่มีกระดานของ " + k);
    if (!/function pickV\(\)/.test(st) || !/pickV\(\);/.test(st)) bad.push("ไม่มีหน้าเลือกบทบาทก่อนเข้าสู่ระบบ");
    if (!/สิทธิ์จริงผูกกับบัญชีของท่านที่ฐานข้อมูล/.test(st)) bad.push("หน้าเลือกบทบาทไม่ได้บอกว่าการเลือกไม่ใช่การให้สิทธิ์");
    if (!/function roleBarHTML/.test(st) || !/คุณกำลังเข้าระบบในฐานะ/.test(st)) bad.push("ไม่มีแถบบอกบทบาทและขอบเขตบนกระดาน");
    if (!/ยังไม่มีเคสที่ส่งต่อมายังบทบาทนี้/.test(st)) bad.push("ไม่มีสถานะว่างเฉพาะบทบาท");
    if (!/ST\.pick&&ST\.pick!==pr\.role/.test(st)) bad.push("เลือกบทบาทไม่ตรงกับบัญชีแล้วไม่ได้บอกผู้ใช้");
    /* ฐานข้อมูลยังต้องเป็นตัวกรองจริง */
    if (!/r\.destination = public\.cs_my_destination\(\)/.test(sql) || !/public\.cs_is_clinician\(\)/.test(sql))
      bad.push("view my_work ไม่ได้กรองด้วยวิชาชีพของผู้ล็อกอิน");
    if (!/pharmacist:\s*\["mine"/.test(st) || /pharmacist:\s*\[[^\]]*"queue"/.test(st))
      bad.push("วิชาชีพเห็นคิวรวมทั้งพอร์ต");
    req(7, "X-136", "คอนโซลแยกกระดานตามวิชาชีพ เลือกบทบาทก่อนเข้าสู่ระบบ และกรองที่ฐานข้อมูล",
        bad.length ? "FAIL" : "PASS",
        bad.length ? bad.join(" · ")
                   : "หน้าเลือกบทบาท 4 วิชาชีพ + ผู้ประสานงาน · ชื่อกระดานและขอบเขตทุกหน้า · สถานะว่างเฉพาะบทบาท · my_work กรองด้วย cs_my_destination ที่ฐานข้อมูล");
    if (bad.length) finding("HIGH", "X-136", "ทุกวิชาชีพเห็นคิวเดียวกัน หรือผู้ใช้ไม่รู้ว่าตนเห็นข้อมูลขอบเขตไหน",
      "เภสัชกรที่เปิดเจอเคสของนักกายภาพ หรือแพทย์ที่ไม่รู้ว่าหน้าจอนี้แสดงเฉพาะเคสที่ส่งถึงตน จะตัดสินใจบนข้อมูลที่เข้าใจผิด และเป็นการเปิดข้อมูลสุขภาพเกินหน้าที่",
      bad.join(" · "), "คงหน้าเลือกบทบาท แถบบอกขอบเขต และการกรองที่ view my_work · เมนูของวิชาชีพห้ามมีคิวรวมทั้งพอร์ต");
  }
